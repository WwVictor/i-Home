...
+ (YBPopupMenu *)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<YBPopupMenuDelegate>)delegate
{
    ...
    CGPoint relyPoint = CGPointMake(absoluteRect.origin.x + 15, absoluteRect.origin.y + absoluteRect.size.height);
    YBPopupMenu *popupMenu = [[YBPopupMenu alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    }else if ([_titles[indexPath.row] isKindOfClass:[RoomInformationModel class]]) {
        RoomInformationModel *model = _titles[indexPath.row];
        cell.roomInfoModel = model;
